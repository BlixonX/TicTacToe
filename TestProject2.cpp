...
bool validate(char map[7][7])
{
	...
	else if ((map[3][1] == 'O' || map[3][1] == 'X') && map[3][3] == map[3][1] && map[3][5] == map[3][1]) { cout << "The winner is player " << map[3][1] << "!\n"; return true; } //Middle Horizontal
	...
	while (!finish)
	{
		while (input != 32)
		{
			...
			{ 
				for (int x = 0; x < 7; x++)
				{
					if (map[y][x] == '#' && y == posY && x == posX) { cout << "\x1B[30;43m#\x1B[m"; }
					else if (map[y][x] == ' ' && y == posY && x == posX) { cout << "\x1B[30;42m \x1B[m"; }
					else if ((map[y][x] == 'X' || map[y][x] == 'O') && y == posY && x == posX) { cout << "\x1B[30;41m"<< map[y][x] <<"\x1B[m"; }
					else { cout << map[y][x]; }
				}
				cout << endl;
			}
